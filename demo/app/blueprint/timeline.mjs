...
import { getTensionForBeat } from '../generation/generation-config.mjs';
import { applyBeatTitles } from '../../../src/generation/random-generator-structure.mjs';
...
function isAutoGeneratedDialogue(dialogue) {
  const id = String(dialogue?.id || '');
  return id.startsWith('dlg_') || /^D\d+$/i.test(id);
}

function inferPurposeForBeat(beat) {
  const key = String(beat?.key || '').toLowerCase();
  const label = String(beat?.label || '').toLowerCase();

  if (/(twist|revel|find|theme|reward|sho|ki|setup|opening|exposition)/.test(`${key} ${label}`)) return 'exposition';
  if (/(conflict|ordeal|climax|all_is_lost|crisis|bad_guys|pinch|ten|take)/.test(`${key} ${label}`)) return 'conflict';
  if (/(mentor|advice)/.test(`${key} ${label}`)) return 'advice';
  if (/(threshold|decision|break_into|plot_turn|go|return)/.test(`${key} ${label}`)) return 'decision';
  if (/(resolution|denouement|final_image|ketsu)/.test(`${key} ${label}`)) return 'resolution';
  if (/(midpoint|rising|tests|search|fun_and_games|approach)/.test(`${key} ${label}`)) return 'confrontation';
  return 'exposition';
}

function inferToneForPurpose(purpose) {
  const toneByPurpose = {
    exposition: 'neutral',
    conflict: 'tense',
    advice: 'warm',
  ...
  return toneByPurpose[purpose] || 'neutral';
}
...
function buildParticipantsForScene(scene) {
  const refs = (scene?.children || []).filter((child) => child?.type === 'character-ref' && child?.refId);
  const participants = [];
  if (refs[0]?.refId) participants.push({ characterId: refs[0].refId, role: 'speaker' });
  if (refs[1]?.refId) participants.push({ characterId: refs[1].refId, role: 'listener' });
  return participants;
}

function ensureDialogueRef(scene, dialogueId) {
  scene.children = Array.isArray(scene.children) ? scene.children : [];
  const exists = scene.children.some((child) => child?.type === 'dialogue-ref' && child?.refId === dialogueId);
  if (exists) return;
  scene.children.push({
    id: genId('ref'),
    type: 'dialogue-ref',
...
function removeDialogueRef(scene, dialogueId) {
  if (!Array.isArray(scene?.children)) return;
  scene.children = scene.children.filter((child) => !(child?.type === 'dialogue-ref' && child?.refId === dialogueId));
}

function syncAutoDialoguesForArc(arcBeats, orderedScenes) {
  const beatByKey = new Map((arcBeats || []).map((beat) => [beat.key, beat]));
  const mappingsBySceneId = new Map();

  for (const mapping of state.project.blueprint.beatMappings || []) {
    if (!mapping?.sceneId || !mapping?.beatKey) continue;
    if (!mappingsBySceneId.has(mapping.sceneId)) mappingsBySceneId.set(mapping.sceneId, []);
    mappingsBySceneId.get(mapping.sceneId).push(mapping);
  }

  const dialogues = Array.isArray(state.project.libraries.dialogues) ? state.project.libraries.dialogues : [];
  const autoDialoguesBySceneId = new Map();
  const manualDialogues = [];

  for (const dialogue of dialogues) {
    if (isAutoGeneratedDialogue(dialogue) && dialogue?.location?.sceneId) {
      const sceneId = dialogue.location.sceneId;
      if (!autoDialoguesBySceneId.has(sceneId)) autoDialoguesBySceneId.set(sceneId, []);
      autoDialoguesBySceneId.get(sceneId).push(dialogue);
    } else {
      manualDialogues.push(dialogue);
    }
  }

  const rebuiltAutoDialogues = [];

  for (const { chapter, scene } of orderedScenes) {
    const mappings = mappingsBySceneId.get(scene.id) || [];
    const existing = autoDialoguesBySceneId.get(scene.id) || [];

    for (let index = 0; index < mappings.length; index += 1) {
      const mapping = mappings[index];
      const beat = beatByKey.get(mapping.beatKey) || { key: mapping.beatKey, label: mapping.beatKey };
      const purpose = inferPurposeForBeat(beat);
      const participants = buildParticipantsForScene(scene);
      const dialogue = existing[index] || {
        id: genId('dlg'),
        exchanges: []
      };

      rebuiltAutoDialogues.push({
        ...dialogue,
        purpose,
        participants,
        tone: inferToneForPurpose(purpose),
        tension: mapping.tension ?? getTensionForBeat(mapping.beatKey),
      ...
      ensureDialogueRef(scene, dialogue.id);
    }

    for (let index = mappings.length; index < existing.length; index += 1) {
      removeDialogueRef(scene, existing[index].id);
    }
  }

  state.project.libraries.dialogues = [...manualDialogues, ...rebuiltAutoDialogues];
}

function rebuildBeatMappingsForArc(arcKey) {
  const arc = getArc(arcKey);
  const beats = arc?.beats || [];
  const orderedScenes = getOrderedScenes();

  if (!beats.length || !orderedScenes.length) {
    state.project.blueprint.beatMappings = [];
    state.project.blueprint.tensionCurve = [];
    state.project.libraries.emotionalArc = [];
    return;
  }

  const lastSceneIndex = Math.max(0, orderedScenes.length - 1);
  state.project.blueprint.beatMappings = beats.map((beat) => {
    const normalizedPosition = Number.isFinite(beat.position) ? beat.position : 0;
    const targetSceneIndex = Math.min(
      lastSceneIndex,
      Math.max(0, Math.floor(normalizedPosition * lastSceneIndex))
    );
    const target = orderedScenes[targetSceneIndex] || orderedScenes[lastSceneIndex];
    return {
      beatKey: beat.key,
  ...
  state.project.blueprint.tensionCurve = beats
    .filter((_, index) => index % 2 === 0)
    .map((beat) => ({
      position: beat.position,
      tension: getTensionForBeat(beat.key)
    }));
  syncAutoDialoguesForArc(beats, orderedScenes);

  const hooks = state.project.blueprint.hooks;
  if (hooks && typeof hooks === 'object') {
    if (hooks.opening && typeof hooks.opening === 'object') {
      const openingBeat = state.project.blueprint.beatMappings[0]?.beatKey || '';
      hooks.opening.targetBeatKey = openingBeat;
      if (!hooks.opening.targetSceneId) {
        hooks.opening.targetSceneId = state.project.blueprint.beatMappings[0]?.sceneId || '';
      }
    }

    if (Array.isArray(hooks.mid)) {
      hooks.mid = hooks.mid.map((hook) => {
        const sceneId = hook?.targetSceneId || '';
        const mappedBeatKey = beatKeyBySceneId.get(sceneId) || '';
        return {
          ...hook,
          targetBeatKey: mappedBeatKey
        };
      });
    }
  }

  const validBeatKeys = new Set(beats.map((beat) => beat.key));
  state.project.libraries.emotionalArc = (state.project.libraries.emotionalArc || [])
    .filter((item) => validBeatKeys.has(item.beatKey));

  applyBeatTitles(state.project.structure, state.project.blueprint.beatMappings);
}
...
function attachEventListeners() {
  ...
  if (arcSelect) {
    arcSelect.addEventListener('change', (e) => {
      const arcKey = e.target.value;
      setBlueprintArc(arcKey);
      rebuildBeatMappingsForArc(arcKey);
      generateCNL();
      render();
    });
  }
